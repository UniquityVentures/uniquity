				...
				&components.FormListenBoostedPost{
					...
					Children: []components.PageInterface{
						&components.FormComponent[AccountingPreferences]{
							...
							Subtitle:      "Go text/template for posted invoice numbers when a draft has no number. Variables: FISCAL_CODE, YY, YYYY, POSTED_SEQ (next posted row id). Example: INV-{{.YYYY}}-{{.POSTED_SEQ}}",
